...
init([FlushIntervalMs, GraphiteHost, GraphitePort, VmMetrics, PathPrefix]) ->
    ...
    {ok, Tref} = timer:apply_interval(FlushIntervalMs, gen_server, cast, [?MODULE, flush]),
...
handle_cast({gauge, Key, Tags, Value0}, State) ->
    Value = {Value0, num2str(unixtime())},
    case ets:lookup(statsdgauge, {Key, Tags}) of
        []                     -> ets:insert(statsdgauge, {{Key, Tags}, [Value]});
        [{{Key, Tags}, Values}] -> ets:insert(statsdgauge, {{Key, Tags}, [Value | Values]})
    end,
    {noreply, State};

handle_cast({increment, Key, Tags, Delta0, Sample}, State) when Sample >= 0, Sample =< 1 ->
    Delta = Delta0 * ( 1 / Sample ), %% account for sample rates < 1.0
    case ets:lookup(statsd, {Key, Tags}) of
        []                          -> ets:insert(statsd, {{Key, Tags}, {Delta,1}});
        [{{Key, Tags},{Tot,Times}}] -> 
            ets:insert(statsd, {{Key, Tags}, {Tot+Delta, Times+1}}),
            ok
    end,
    {noreply, State};

handle_cast({timing, Key, Tags, Duration}, State) ->
    case gb_trees:lookup({Key, Tags}, State#state.timers) of
        none         -> {noreply, State#state{timers = gb_trees:insert({Key, Tags}, [Duration], State#state.timers)}};
        {value, Val} -> {noreply, State#state{timers = gb_trees:update({Key, Tags}, [Duration|Val], State#state.timers)}}
    end;
...
send_to_graphite(Msg, State) ->
    ...
    case gen_tcp:connect(State#state.graphite_host,
        ...
        {ok, Sock} ->
            ...
            gen_tcp:close(Sock),
            gen_server:cast(?MODULE, {tx_bytes, iolist_size(Message)}),
            ok;
        E ->
...
key2str(K) when is_atom(K)   -> atom_to_list(K);
key2str(K) when is_binary(K) -> key2str(binary_to_list(K));
...
do_report(All, Gauges, State) ->
    ...
    %% REPORT TO GRAPHITE
    case NumCounters + NumTimers + NumGauges + NumVmMetrics of
        0 -> nothing_to_report;
        NumStats ->
            ...
            [{num_stats, NumStats}, {tx_bytes, State#state.tx_bytes}]),
...
do_report_timers(TsStr, State) ->
    ...
    Msg = lists:foldl(
        fun({{Key, Tags}, Vals}, Acc) ->
                ...
                %% Build stats string for graphite
                Startl          = [ State#state.path_prefix, ".", KeyS],
                Endl            = [" ", TsStr, "\n"],
                Fragment        = [ [Startl, Name, " ", num2str(Val), Endl] || {Name,Val} <-
                                  [ {Tags ++ ";agg=mean", Mean},
                                    {Tags ++ ";agg=upper", Max},
                                    {Tags ++ ";agg=upper_"++num2str(PctThreshold), MaxAtThreshold},
                                    {Tags ++ ";agg=lower", Min},
                                    {Tags ++ ";agg=count", Count}
                                  ]],
                [ Fragment | Acc ]
        end, [], Timings),
    {Msg, length(Msg)}.
        ...
        fun({{Key, Tags}, Vals}, Acc) ->
            ...
            Fragments = lists:foldl(
                fun ({Val, TsStr}, KeyAcc) ->
                    %% Build stats string for graphite
                    Fragment = [
                        State#state.path_prefix, ".", KeyS, Tags, " ",
                        io_lib:format("~w", [Val]), " ",
                        TsStr, "\n"
                    ],
                    [ Fragment | KeyAcc ]
